...
class CanvAscii {
  ...
  updateRotation(time: number) {
    // Add automatic movement instead of just mouse-based
    const autoX = Math.sin(time * 0.5) * 0.4;
    const autoY = Math.cos(time * 0.3) * 0.4;
    
    // Blend mouse movement with automatic movement
    const mouseInfluence = 0.3;
    const autoInfluence = 0.7;
    
    const x = map(this.mouse.y, 0, this.height, 0.5, -0.5) * mouseInfluence + autoX * autoInfluence;
    const y = map(this.mouse.x, 0, this.width, -0.5, 0.5) * mouseInfluence + autoY * autoInfluence;

    this.mesh.rotation.x += (x - this.mesh.rotation.x) * 0.05;
    this.mesh.rotation.y += (y - this.mesh.rotation.y) * 0.05;
    
    // Add subtle position movement
    this.mesh.position.x = Math.sin(time * 0.4) * 2;
    this.mesh.position.y = Math.cos(time * 0.6) * 1.5;
  }
